...
export default function CreateMatch({ onClose }) {
  ...
  const create = async () => {
    ...
    // Wait for the match to be added via AppContext API request
    await addMatch(m);
    setStatus('done');
  };
      ...
      <div className="modal-sheet">
        ...
        <div style={{ ...S, display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div>
            ...
            {/* Changed min from 2 to 1 and max from 22 to 30 as requested */}
            <input type="range" className="range" min={1} max={30} value={form.players} onChange={e => setForm(f => ({ ...f, players: +e.target.value }))} />
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
              {/* Changed labels to match limits */}
